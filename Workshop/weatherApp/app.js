...
let weatherApiService = {
    ...
    getWeatherData: async function (city) {
        ...
        try {
            ...
            uiService.displayHourlyStats(data.list);
            setHourlyStatService.sortHourlyStats(data.list);
        } catch (error) {
...
setHourlyStatService =
{
    sortHourlyStats: function (hourlyData) {

        document.getElementById("sortTempAsc").addEventListener("click", () => {
            let sortedData = hourlyData.slice().sort((a, b) => a.main.temp - b.main.temp);
            uiService.displayHourlyStats(sortedData);
        });

        document.getElementById("sortTempDesc").addEventListener("click", () => {
            let sortedData = hourlyData.slice().sort((a, b) => b.main.temp - a.main.temp);
            uiService.displayHourlyStats(sortedData);
        });

        document.getElementById("sortHumidityAsc").addEventListener("click", () => {
            let sortedData = hourlyData.slice().sort((a, b) => a.main.humidity - b.main.humidity);
            uiService.displayHourlyStats(sortedData);
        });

        document.getElementById("sortHumidityDesc").addEventListener("click", () => {
            let sortedData = hourlyData.slice().sort((a, b) => b.main.humidity - a.main.humidity);
            uiService.displayHourlyStats(sortedData);
        });
    }
}
...
let uiService = {
    ...
    displayHourlyStats: function (hourlyData) {
        let tableBody = document.getElementById("hourlyWeatherBody");
        tableBody.innerHTML = "";
